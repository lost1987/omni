<?php
    ...
    class Lottery extends Controller
    {
        ...
        function index()
        {
            ...
            $master_prize['inventory'] = 0;//强制库存为0 如果要开启大奖获得 删除此行代码
            ...
            $master_prize = array();//强制库存为0 如果要开启大奖获得 删除此行代码
            $other_prizes[3]['chance'] = 0;
            $other_prizes[3]['inventory'] = 0;
            ...
            try {
                ...
                if ( $result['level'] == 1 || $result['level'] == 2 ) {
                    ...
                    $data = array(
                        'action_type' => UserResource::ACTION_LOTTERY,//资源变动类型:玩家抽奖
                        'tool_type' => 0,
                        'price_type' => Tools::array_toggle_fetch_value($result['price_type'],$this->config->web['price_type_columns']),
                        'price' => 0,
                        'tool' => $result['price'],
                        'uid' => $this->_uid
                    );
