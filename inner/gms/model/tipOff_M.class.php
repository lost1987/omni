...
class TipOff_M extends AdminModel{
    ...
    function lists($start=null,$count=null){
        ...
        $sql = "SELECT a.*,b.reporter_name,b.assign_to,b.result,b.handler_type,b.handle_time  FROM index_tipoff a   LEFT JOIN index_handleresult b ON a.handler_id = b.id  {$this->_condition}  ORDER BY a.create_at DESC $limit";
    ...
    function num_rows(){
        $sql = "SELECT COUNT(*) as num FROM index_tipoff a  LEFT JOIN index_handleresult b ON a.handler_id = b.id {$this->_condition}";
        $this->_game_server->execute($sql);
        $this->_condition = '';
    ...
    function read($id){
        $sql = "SELECT * FROM index_tipoff WHERE id  = ?";
