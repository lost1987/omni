...
class FeedBack_M extends AdminModel{
    ...
    function num_rows(){
        $sql = "SELECT COUNT(*) as num FROM index_feedback a  LEFT JOIN index_handleresult b ON a.handler_id = b.id {$this->_condition}";
        $this->_game_server->execute($sql);
        $this->_condition = '';
    ...
    function read($id){
            $sql = "SELECT * FROM index_feedback WHERE id  = ?";
