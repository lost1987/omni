...
class Page
{
    ...
    protected  $use_tag_li; //是否使用li包裹A标签

    protected  $li_classname; // 当前页面li标签上的css类名

    protected  $a_classname; // 当前页面a标签上的css类名

    protected $query_string;//当前的查询参数 "?"后面的一串查询字符串
    ...
    public function __construct($data = array())
    {
        ...
        $this->li_classname = !empty($data['li_classname']) ? $data['li_classname'] : '';
        $this->a_classname = !empty($data['a_classname']) ? $data['a_classname'] : '';
        $this->use_tag_li = !empty($data['use_tag_li']) ? $data['use_tag_li'] : false;
        $this->query_string = !empty($data['query_string']) ? $data['query_string'] : '';
    ...
    protected function _get_link($page,$text)
    {
        switch ($this->method) {
            case 'ajax':
                ...
                $str = '<a onclick="' . $this->ajax_func_name . '(\'' . $page . '\''.$parameter.')" href="javascript:void(0)">' . $text . '</a>' ;
                if($this->use_tag_li)
                    $str = '<li>' . $str .'</li>';
                $str .= "\n";
                return $str;
            ...
            case 'html':
                ...
                if(!empty($this->query_string)) $url .= '?'.$this->query_string;
            ...
            default:
                $str = '<a href="' . $this->_get_url($page) . '">' . $text . '</a>';
                if($this->use_tag_li)
                    $str = '<li>' . $str .'</li>';
                $str .= "\n";
                return $str;
    ...
    protected function show_1()
    {
        ...
        for ($i = $begin; $i <= $begin + $plus * 2;$i++)
        {
            ...
            {
                if($this->use_tag_li)
                    $return .= '<li class="'.$this->li_classname.'"><a class="'.$this->a_classname.'">'.$i.'</a></li>';
                else
                    $return .= '<a class="'.$this->a_classname.'">'.$i.'</a>';
                $return .= "\n";
            }
            else
            {
